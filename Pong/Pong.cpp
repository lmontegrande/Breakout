...
void Pong::play() {
	...
	while (window.isOpen()) {
		...
		gameObjects._ball->directionVector2f = Vector2f(cos(randomDirection), sin(randomDirection));
...
int handleCollisions(GameObjectsContainer &gameObjects, RenderWindow &window, Sound &ballSound, Sound &obstructionSound) {
	...
	// Hit Left
	if (ballPosition.x <= 0) {
		gameObjects._ball->directionVector2f.x = abs(gameObjects._ball->directionVector2f.x);
		return 2;
	}
	// Hit Right
	...
	if (ballPosition.y - ballRadius <= 0) {
		gameObjects._ball->directionVector2f.y = abs(gameObjects._ball->directionVector2f.y);
		ballSound.play();
	}
	// Hit Bottom
	...
	if (circlesColliding(gameObjects._ball->circleShape, gameObjects._obstruction->circleShape)) {
		if (gameObjects.isCircleCollisionExit) {
			if (gameObjects.lastPaddleToHit != -1) {
				...
				gameObjects._ball->directionVector2f = deltaVector / (sqrtf(deltaVector.x * deltaVector.x + deltaVector.y * deltaVector.y));
			}
